/**
 * Stream Manager for Agent Connect Server
 * Adapted from NavigoAI implementation for our WebSocket proxy architecture
 */

class StreamManager extends SoundHandler {
    constructor(serverUrl = null) {
        // Auto-detect server URL if not provided
        const finalServerUrl = serverUrl || `ws://${window.location.host}`;
        
        // Call parent constructor
        super(finalServerUrl);
    ...
    async handleMessage(message) {
        // Handle different message types based on our server's format
        if (message.error) {
    ...
    async attemptReconnect() {
        if (this.reconnectAttempts >= this.maxReconnectAttempts || this.connectionMutex) {
            console.error('Max reconnection attempts reached or connection in progress');
            return;
        }
    ...
    async playSound(base64Audio) {
        try {
            ...
            if (!this.audioContext || this.audioContext.state === 'closed') {
                ...
                // Limit the number of contexts we track
